...
Every nucleotide is an independent observation.
...
A significant problem of storing probabilities at the level of individual nucleotides is that there are an enormous number of possible sequences ($5^\ell$).
...
Thus, we expect that $\ell$ exceeds 30,000, such that the total number of possible sequences is greater than $10^{20970}$.
...
This compact and approximate representation drastically reduces the space of all possible sequences, \ie $m\ll 5^\ell$.
...
$$
then there are $2\times 3 \times 2^3 \times 3 = 144$ possible sequences. 
...
The quality score $Q$ is directly related to the error probability: $\epsilon = 10^{-Q/10}$ \cite{ewingBaseCallingAutomatedSequencer1998}, where $Q$ typically ranges between 1 and 60.
...
The aligned outputs are frequently recorded in the SAM format, which is a tabular data format with application-specific encodings such as the CIGAR serialization of pairwise alignments.
