...
- Nomenclature: "sequence-level uncertainty sequence" or "genome likelihood"?
...
In \ref{fig:covidcalls}, each row represents one SAM file. Each bar represents one lineage assignment from pangolin. The width of the bars represents the sum of the weights (genome likelihoods) of sampled sequences. Bars are subdivided to visualize these weights.

**Figure 1: Best and worst case scenario bar plots, conseq coloured.**
...
Initial analyses used pangolin version 2.0.8, which used multinomial logistic regression, rather than decision trees, to assign lineages. This procedure resulted in bootstrap support below 1. 
