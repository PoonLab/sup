...
The clock rate for SARS-CoV-2 is commonly estimated as a fixed rate near 0.001 mutations per site per year (specific estimates will be given below).
Using similar resampling methods as above (although without indels), we estimate a clock rate for trees estimated from each of 50 resamples, as well as for the tree estimated based on the reported conseqs.
...
We note that it is not ideal to remove data, but the error rate # TODO: finish this sentence.

We filtered the sequences so that they all had similar overall uncertainty, as defined by the sum of \nps'.
This filtering was done according methods of statistical process control, where the 
...
Figure \ref{heydevanputafigurereferencehere} demonstrates that the estimated evolutionary rates have an average close to the rate estimated from the conseqs, but there is significantly more variation.
Rate estimates from @ducheneTemporalSignalPhylodynamic2020, @choudharySevereAcuteRespiratory2021, @songGenomicEpidemiologySARSCoV22021, @niePhylogeneticPhylodynamicAnalyses2020, and @geidelbergGenomicEpidemiologyDensely2021 are also labelled on the plot, indicating that the rates from our re-sampled sequences are in line with other published rates.
