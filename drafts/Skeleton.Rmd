...
For example, mixed peaks in sequence chromatograms produced from dye terminator sequencing by capillary electrophoresis are assigned standard IUPAC codes (\eg Y for C or T, R for A or G) when the base calling program cannot determine which base is dominant.
...
For example, @depristoFrameworkVariationDiscovery2011 and @gompertHierarchicalBayesianModel2011a incorporate adjusted Phred scores into a likelihood framework to generate more accurate estimates of population-level allele distributions; this approach has subsequently been used to develop new estimators for population genetic diversity (@fumagalliQuantifyingPopulationGenetic2013a).
...
Other than filtering data, the uncertainty present in the sequences are seldom propagated to downstream analyses.
...
Another exception can be found in @kuhnerCorrectingSequencingError2014, who incorporate the quality scores into the calculation of a phylogenetic tree, and found that incorporation of errors makes the inferred branch lengths much closer to the true (simulated) branch lengths.
...
Their method incorporates Position Weight Matrices (PWM), which are obtained from text file outputs of Solexa/Illumina sequencing platforms, into a method of scoring matches against a reference genome. 
Unlike systems where multiple matches are either discarded, condensed to a single match (first match or at random), or aligned to all matches, their algorithm assigns a matching score to each match based on the probability scores which reduces the bias in the positional alignment. 


Here we present a simple general-purpose framework that can be incorporated into any analysis of genetic sequence data.
...
In both frameworks, the sequence of nucleotides from a biological sample is not treated as a single unambiguous observation (known without error), but rather as a collection of possible sequences.
...
Its elements represent the probability that a nucleotide is at given position:
...
This file type is able to be used by RAxML-NG in TODO: Finish this sentence.
Our description differs in that it TODO: Re-read reference, finish this sentence.

TODO: This next section seems like too much of a tutorial.
- The examples could be shortened greatly.
- The notation is not used anywhere else.
...
Insertions (`I` operations) are temporarily excluded from the aligned sequence.

Insertions are cached in a separate data structure ($\mathcal{I}$) that records the inserted sequence, quality string, and position.
At each insertion site the number of insertions and the length of each insertion at that site are recorded. 
For each site $j$ and each insertion length ($\ell_{i,j}$), a version of the uncertainty matrix is constructed, $\nps'_{ij}$, which contains the sums of $\epsilon$ for each insertion. 
Since we can't know how many bases might have been read at the position of insertions (since we can't know how many short reads would have started or ended at or before the insertion sies), the coverage at the position prior to the insertion is used in place of the coverage at the insertion site.
When using this data structure for re-sampling the sequence, the number of insertions is sampled according to the coverage at the previous submission and the number of insertions of each length (\eg if there were 10 insertions of length one and the coverage was 40, an insertion would be sampled 25\% of the time).
Conditional on the number of insertions, the inserted bases are sampled according to the same sampling procedure used for $\nps'$. 



<!---
After all reads are processed, we initialize an empty $5\times\ell$ matrix $\nps'$, where $\ell$ is initialized to the length of the longest aligned sequence excluding insertions.
We map each base in the aligned sequences to the corresponding row and column in $\nps'$ and increment the value at that position by $1-\epsilon$ and $\epsilon/3$ to the other entries in the column, where $\epsilon$ is derived from the base quality score.
...
TODO: Re-read and edit heavily, possibly re-write or remove

Suppose we wish to use a single sequence to estimate some parameter $\theta$ (for instance, the p-value for whether a sequence is a particular variant or the lineage assignment for grafting to a pre-computed tree) and we are interested in the effect that uncertainty has on this estimate.
...
At some point, it will be uncertain whether making a different substitution will have a larger effect on the likelihood than adding a substitution than one made in another sequence.
...
The sequences can be ordered according to their uncertainty, with the most likely sequence having uncertainty $a_{(1)}$, the second-most likely sequence $a_{(i)}$, etc.
...
TODO: $a_{(0)}$ has already been used! Maybe use primes?
...
From this phylogeny, we simulate uncertainty (FASTQ-style) at four different parameter combinations, based on those found in @zaniniPopulationGenomicsIntrapatient2015. TODO: Fix Zanini reference
...
The mean values of this beta distribution were chosen to represent reasonable amounts of error, then the standard deviations were chosen based on the order of magnitude of the mean, then the corresponding shape parameters were found (see Figure \ref{fig:btshp}). TODO: Fix figure reference.
New sequences were simulated from this uncertainty based on sampling from the uncertainty matrix defined by these error probabilities, with the remaining probability distributed uniformly across the other bases (see Section \ref{fdskfdsf}). TODO: Fix section reference.
...
The resultant set of sequences is used to estimate a tree using RAxML @stamatakisRAxMLVersionTool2014. TODO: Fix RAxML reference (probably re-export the bibtex file).
...
23 out of 300 files were labelled incomplete due to having too few reads (possibly because the download timed out) or not containing a CIGAR string.
...
Since pangoLEARN is a pre-trained model, the analysis is not computationally burdensome.
Sampling 10,000 different sequences from a call probability matrix is reasonable, even on a mid-range consumer laptop.

For this analysis we use the most basic resampling strategy.
...
In \ref{fig:covidcalls}, each row represents resamples from one SAM file. TODO: Fix figure references.
Each bar represents one lineage assignment from pangolin.
The width of the bars represents the sum of the weights (genome likelihoods) of sampled sequences.
Bars are subdivided to visualize these weights.
...
Results for all lineages can be found in Table \ref{tab:pango} in the Appendix. TODO: Why are the figure references bad?
...
Figure \ref{fig:covidcalls} shows that the consensus sequence almost always is assigned the same lineage as the majority of the resamples.
However, the resamples never agree 100\%, and the proportion of resamples that agree with the conseq can be as low as 59%.
This is in contrast to the 100\% bootstrap support that pangolin applied to all sequences analysed (or 0\% for the ones labelled "None" in Table \ref{tab:pango}).

The full results are in Table \ref{tab:pango}.
There are no cases where the consensus sequence is different from the majority of resamples, but the proportion of resamples with the same lineage as the consensus sequence is very rarely 100\% and can be as low as 22.09\%.
...
The clock rate was estimated using TreeTime (reference). TODO: find TreeTime reference.
...
Re-sampling allows a more honest appraisal of the variance in the estimates (or provides a reasonable prior distribution in a Bayesian setting), while comparing results for the most likely sequences provide a measure of robustness to sequence uncertainty.

Our proposed methods can result in a drastic increase in compuatational expense.
...
We cannot account for systematic errors, such as those present due to human errors (\eg mis-calibrated machines or that one lab in @blank that kept giving the same mutations).
Our method allows for adjustments of the base call quality score, as well as more sophisticated definitions genome likelihoods.
