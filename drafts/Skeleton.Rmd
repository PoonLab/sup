...
## Sequence-level Uncertainty (seqLevelUncertainty)

Suppose we wish to use the sequences to estimate some parameter $\theta$ and we are interested in the affect that uncertainty has on this estimate. Computational burden can be partially alleviated by incorporating sequence-level uncertainty. 

To start with, a cut-off probability can be set for the nucleotide uncertainty. Any base that has a 1/1000 error rate (its entry in $\nps$ is larger than 99.9%) can be set to a 0% error rate. With this change, the sequence uncertainty for all possible sequences (or all bioligically feasible sequences) can be calculated. 

The sequences can be ordered according to their uncertainty, with the uncertainties acting as weights. 
