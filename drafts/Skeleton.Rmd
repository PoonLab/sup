...
Another exception can be found in @kuhnerCorrectingSequencingError2014 (TODO: Fix reference), who incorporate the quality scores into the calculation of a phylogenetic tree, and found that incorporation of errors makes the inferred branch lengths much closer to the true (simulated) branch lengths.
...
The aligned outputs are frequently recorded in the tabular Sequence Alignment/Map (SAM) format (@li2009sequence).
...
Instead, each insertion is cached in a separate data structure ($\mathcal{I}$) that records the inserted sequence, quality string, and position.
...
There are several programs that provide methods to convert a SAM file into a consensus FASTQ file (@liAdjustQualityScores2004, @keithSimulatedAnnealingAlgorithm2002, @liMappingShortDNA2008).
...
From this phylogeny, we simulate uncertainty (FASTQ-style) at four different parameter combinations, based on those found in @Zanini. TODO: Fix Zanini reference
...
The resultant set of sequences is used to estimate a tree using RAxML \citep{stamatakisRAxMLVersionTool2014}. TODO: Fix RAxML reference (probably re-export the bibtex file).
...
We looked at the Robinson-Foulds distance, where the distance is the number of splits that exist in one tree but not the other  \citep{robinsonComparisonPhylogeneticTrees1981}, weighted Robinson-Foulds distance, which adds the branch lengths of the splits rather than counting each split as one unit, and the Kuhner-Felsenstein distance \citep{SimulationComparisonPhylogeny1994}, which measures the sum of the squared differences in branch lengths between trees.
...
Our method allows for adjustments of the base call quality score (such as those in @blank), as well as more sophisticated definitions genome likelihoods (such as those in @blank2).
