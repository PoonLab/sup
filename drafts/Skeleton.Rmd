...
If working with RNA, then we must use reverse transcription to convert the RNA into DNA.
For some next-generation sequencing (NGS) applications, we have to generate a sequencing library by random shearing of nucleic acids into fragments that are ligated onto special `adaptors'. 
...
In addition, the short reads produced by NGS platforms need to be aligned &mdash; either by alignment against a reference genome, *de novo* assembly, or a combination of the two &mdash; to reconstruct the complete sequence using one or more bioinformatic programs.
...
In some cases, the variation induced by measurement error is directly quantified and encoded into the output.
For example, ambiguous base calls from sequence chromatograms are assigned ambiguity codes (\eg Y for C or T, R for A or G) when the base calling program cannot determine which base is dominant. 
...
@kuoEAGLEExplicitAlternative2018 use genome likelihoods to perform hypothesis tests that evaluate whether a given genome sequence is consistent with a given alternative sequence after accounting for the errors. 
As with any parametric model-based approach, genome likelihoods use assumptions about the errors in order to make conclusions about the underlying patterns. 
...
Methods to reconstruct phylogenies usually leave out the uncertainty complexity and settle for sequences composed of the most frequent nucleotides and/or ignore ambiguity codes (with some exceptions, \eg @depristoFrameworkVariationDiscovery2011).
