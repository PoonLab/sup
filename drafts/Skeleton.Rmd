...
One notable exception is @oraweAccountingUncertaintyDNA2015, who suggest methods for propagating sequence-level uncertainty into determining whether two subjects have the same alleles, as well as estimating confidence intervals for allele frequencies.
...
The majority of these possible sequences are not biologically plausible.
...
Large genomes, such as SARS-CoV-2, will result in vanishingly small probabilities for all sequences, and thus calculations on the log scale may be necessary to reduce the chance of numerical underflow.


### Deletions and Insertions

By construction, the \nlps must be defined with its longest possible length.
Deletions are naturally modelled with our representation but insertions have to be modelled using deletion probability.
...
The aligned outputs are frequently recorded in the SAM format, which is a tabular data format with application-specific data types.
Each row represents a short read, including the optimal placement of the read with respect to the reference sequence (as an integer offset), and an application-specific serialization of the edit operations required to align the read to the reference, called the compact idiosyncratic gapped alignment report (CIGAR, @li2009sequence).


We suggest the following algorithm to construct the nucleotide-level probabilistic sequence from the contents of a SAM alignment file.
First, we initialize an empty $5\times\ell$ matrix $\nps'$.
Iterating through each row of the SAM file, the position and CIGAR string determine the location of each base in the alignment.
We map each base to the corresponding row and column in $\nps'$ and increment the value at that position by $1-\epsilon$ and $\epsilon/3$ to the other entries in the column, where $\epsilon$ is derived from the base quality score.
By this construction, the sum of each column represents the coverage at that location.
Dividing each entry of $\nps'$ by the sum of that column results in $\nps$.
For our proposed propagation methods, it is convenient to work with $\nps'$ until probabilities are necessary.
...
Given the consensus base and its associated quality score at position $j$, we must assume that the other bases are all equally likely with probability $\epsilon_j/3$.
...
Re-sampling can either be done simply based on the nucleotide-level uncertainty as a multinomial distribution.
...
Thes values can be used as the parameters in the Dirichlet prior, from which base probabilities can be sampled.
...
In a maximum likelihood framework, this is similar to bootstrapping.
In fact, the ultimate effect of this would be to decrease the bootstrap confidence to a level that is more in line with the actual uncertainty in the data.

In a Bayesian framework, this could be incorporated as prior distribution on each nucleotide.
For large collections of large sequences, this increases the dimensionality dramatically.
...
From this phylogeny, we simulate uncertainty (FASTQ-style) at four different parameter combinations, based on those found in @Zanini.
...
The resultant set of sequences is used to estimate a tree using RAxML \citep{stamatakisRAxMLVersionTool2014}.
...
In \ref{fig:covidcalls}, each row represents resamples from one SAM file.
