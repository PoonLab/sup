...
.tree.to.igraph <- function(tree) {
    ...
    g <- igraph::set.edge.attribute(g, 'length', value=tree$edge.length)
    g <- igraph::set.vertex.attribute(g, 'production', value=.get.productions(g))
    g <- igraph::set.vertex.attribute(g, 'bl', value=.get.branchlengths(g))
    # store sum-of-squares for branch lengths
    g <- igraph::set.vertex.attribute(g, 'ssq.bl', 
                              value=sapply(
                                  get.vertex.attribute(g, 'bl'), 
                                  .ssq)
    )
    g <- igraph::set.vertex.attribute(g, 'children', value=.get.children(g))
    g
}
