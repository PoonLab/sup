...
# https://github.com/PoonLab/Coevolution/blob/master/src/phyloK2.py
        ...
        for ni, n1 in enumerate(nodes1):
            ...
            for n2 in nodes2:
                if n1.production == n2.production:
                    if self.labelFilter is not None:
                        res1 = math.exp(-1. / self.gaussFactor * (n1.sqbl + n2.sqbl - 2 * sum(
                            [(n1.bl[i] * n2.bl[i]) for i in range(len(n1.bl))]
                        )))
                        res2 = math.exp(-1. / self.gaussFactor * (n1.sqbl + n2.sqbl - 2 * sum(
                        ...
                        res = self.decayFactor * math.exp(-1. / self.gaussFactor * (n1.sqbl + n2.sqbl - 2 * sum(
                            [(n1.bl[i] * n2.bl[i]) for i in range(len(n1.bl))]
                        )))
