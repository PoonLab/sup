...
For next-generation sequencing (NGS) protocols, we have to generate a sequencing library for instance by random shearing of nucleic acids into fragments that are ligated onto special 'adaptors'. TODO: help wanted: reference for this?
...
The GISAID accession numbers for the seqeunces we used are provided in the appendix. TODO: (Devan) Other info about the files?
TODO: (Devan) add the accessions to the appendix
...
TODO: Maybe move this to the figure legend? Or just parts of it?
In \ref{fig:covidcalls}, each row represents 10,000 resamples from one SAM file.
...
There were 95 sequences total, but we only plotted the 45 accession numbers where the second most common lineage designation had more than 250 observations.
...
The proportion of resamples with the same lineage as the consensus sequence is very rarely 100\% and can be as low as 22.09\%. TODO: (Devan) Provide accession number.
It is noteworthy that the only times where 100\% of resampled sequences agreed are when the lineage call was ``None'' or for the lineage labelled B.1.1.7, which is a significantly more infectious lineage and is of special concern to health authorities. TODO: (Devan) make sure this is still truel; newer versions of Pangolin use post-processing of B.1.1.7 according to Art.
...
The associated GISAID accession numbers are provided in the Appendix. TODO: (Devan) add accession numbers.
...
TODO: Comment on ordered likelihood conclusions / usefulness as a way of approximating the distribution and evaluating how many mutations it takes before the methods break.
