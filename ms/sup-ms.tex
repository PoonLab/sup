...
The quality score $Q$ is directly related to the error probability: $\epsilon = 10^{-Q/10}$ \cite{xxx} (for the widespread Illumina instruments, the sequencing error probability ranges between $10^{-3.5}$ and  $10^{-1.5}$ \cite{xxx}). The FASTQ file format is the standard representation for combining sequence and observation error.
...
\begin{equation}
\label{eq:basecall}
\pr{\text{observed nucleotide}=X \,\,|\,\, \text{true nucleotide} = X} = 1 - \epsilon
\end{equation}
...
For a given position, the error probability is $\epsilon = 10^{-Q/10}$ (that is its quality score is $Q$) and we assume the probability a deletion happens at this position is $d$.  Conditional on not being deleted, the probability to have the base called is $(1-d)(1-\epsilon)$ and the other three nucleotides can occur with probability $(1-d)\epsilon/3$. 
...
For example, the ambiguity code \sq{Y} is interpreted as

\warning{review this, I think this is wrong:}
 $p(\text{observed nucleotide} = C \,\,|\,\, \text{true nucleotide} = C\, or\, T ) = p(\text{observed nucleotide} = T \,\,|\,\, \text{true nucleotide} = C\, or\, T ) =q/2$ and $p(\text{observed nucleotide} = A \,\,|\,\, \text{true nucleotide} = C\, or\, T ) = p(\text{observed nucleotide} = G \,\,|\,\, \text{true nucleotide} = C\, or\, T ) =(1-q)/2$.
...
where \sq{X} represents non-\sq{A} bases (the order does not matter).
...
\right)^{-1}
\end{equation}
where $\epsilon$ is the observation error probability associated with each observation (obtained from the FASTQ file of the short read).
